if Code.ensure_loaded?(Mint.HTTP) do
  defmodule Dlex.Adapters.HTTP do
    ...
    defp parse_success(:alter, %{"data" => data}) do
      %Dlex.Api.Payload{Data: data}
    end

    defp parse_success(:mutate, %{"data" => %{"uids" => uids, "queries" => queries}} = response) do
      %Dlex.Api.Response{txn: parse_txn(response), uids: uids, json: queries}
    end

    defp parse_success(:query, %{"data" => data} = response) do
      %Dlex.Api.Response{txn: parse_txn(response), json: data}
    end
    ...
    defp parse_txn(_, aborted) do
      %Dlex.Api.TxnContext{aborted: aborted}
    end
